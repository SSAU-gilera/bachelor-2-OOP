...
void Database::print_name(unsigned kard)
{
	...
	unsigned* kard_arr = (unsigned*)malloc(sizeof(unsigned) * num_of_students); 
	for (int i = 0; i < num_of_students; i++) { //переписываем в новый массив номера зачёток по порядку
		kard_arr[i] = index_arr[i].kard;
	}
	unsigned* found = (unsigned*)bsearch(&kard, kard_arr, num_of_students, sizeof(unsigned), cmp_kard); //бинарный поиск
	int idx = (kard_arr - found) / sizeof(unsigned); //находим индекс найдённого номера
	IndexKard idx_kard = index_arr[idx];//получает индексную запись с номером по заданному индексу
	fseek(studentstream, idx_kard.index, SEEK_SET); //ставим указатель на индекс нужной записи
	Student st;
